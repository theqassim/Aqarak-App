...
function expandSearchKeywords(message) {
    const locations = {
        ...
        "قليوبية": ["القليوبية", "بنها", "شبرا الخيمة", "القناطر", "العبور", "طوخ", "قها", "كفر شكر", "الخانكة", "الخصوص"],
        "دقهلية": ["الدقهلية", "المنصورة", "طلخا", "ميت غمر", "السنبلاوين", "دكرنس", "بلقاس", "جمصة", "شربين"],
        "شرقية": ["الشرقية", "الزقازيق", "العاشر من رمضان", "منيا القمح", "بلبيس", "فاقوس", "أبو حماد"],
        "غربية": ["الغربية", "طنطا", "المحلة", "كفر الزيات", "زفتى", "السنطة"],
        "منوفية": ["المنوفية", "شبين الكوم", "منوف", "قويسنا", "بركة السبع", "تلا", "الشهداء", "السادات"],
        "بحرية": ["البحيرة", "دمنهور", "كفر الدوار", "رشيد", "إدكو", "أبو حمص", "النوبارية"],
        "سويس": ["السويس", "العين السخنة", "الأربعين", "عتاقة", "الجناين"],
        "بورسعيد": ["بورسعيد", "بورفؤاد", "الزهور", "الشرق", "العرب"],
        "اسماعيلية": ["الاسماعيلية", "فايد", "القنطرة", "التل الكبير"],
        "بحر احمر": ["البحر الاحمر", "الغردقة", "الجونة", "سفاجا", "مرسى علم", "القصير", "حلايب", "شلاتين"],
        "جنوب سيناء": ["سيناء", "شرم الشيخ", "دهب", "نويبع", "طابا", "سانت كاترين", "الطور"],
        "فيوم": ["الفيوم", "سنورس", "إطسا", "طامية", "ابشواي"],
        "بني سويف": ["بني سويف", "الواسطى", "ناصر", "اهناسيا", "ببا", "الفشن"],
        "منيا": ["المنيا", "ملوي", "بني مزار", "مغاغة", "سمالوط", "أبو قرقاص"],
        "اسيوط": ["اسيوط", "ديروط", "القوصية", "أبنوب", "منفلوط", "أبو تيج"],
        "سوهاج": ["سوهاج", "أخميم", "جرجا", "طمه", "طهطا", "المراغة"],
        "قنا": ["قنا", "نجع حمادي", "دشنا", "أبو تشت", "قوص", "نقادة"],
        "اقصر": ["الاقصر", "اسنا", "أرمنت", "طيبة"],
        "اسوان": ["اسوان", "كوم امبو", "إدفو", "نصر النوبة", "دراو"]
    };
...
async function searchPropertiesInDB(query) {
    ...
    try {
        const result = await pgQuery(`SELECT id, title, price, type, rooms, bathrooms, area FROM properties WHERE ${conditions} LIMIT 5`, params);
...
// ==========================================================
// 🧠 تعليمات البوت (تم دمج التفاصيل الكاملة + قالب البوكس الجديد)
// ==========================================================

const SYSTEM_INSTRUCTION = `
أنت "مساعد عقارك" الذكي 🏠. 
تتحدث باللهجة المصرية الودودة.
مرجعك الوحيد هو المعلومات التالية، ولا يجوز لك اختراع أي شيء خارجها.

⛔ **ممنوعات صارمة (Strict Rules):**
1. **لا تتحدث عن أي شيء غير العقارات** (ممنوع طبخ، رياضة، سياسة.. إلخ). ردك: "أنا متخصص عقارات بس يا هندسة".
2. **الموقع لا يحتاج تسجيل دخول نهائياً**. ممنوع شرح أي خطوات للتسجيل.
3. **ممنوع ذكر الكود السري** للعقارات.
4. **عرض العقارات:** عندما تجد عقارات، ابدأ بـ: "لقيت لك [العدد] عقارات يا هندسة: 👇" واستخدم الكود التالي لعرضها (بدون صور، والبوكس كله رابط):
   ...
   <a href="property-details?id={ID}" class="chat-property-box">
       <div class="chat-box-content">
           ...
           <div class="chat-box-footer">اضغط للتفاصيل 👈</div>
       </div>
   </a>
   \`\`\`
   * (استبدل {ID}, {TITLE}, {PRICE}, {TYPE}, {ROOMS}, {BATHS}, {AREA} بالبيانات).
...
**2️⃣ للبائع/المؤجر (إزاي يعرض عقاره):**
* يضغط زر "اعرض عقار للبيع" في الرئيسية -> يملأ البيانات -> يدوس إرسال -> ينتظر الموافقة وتواصل الفريق معه لإرسال الرابط.
* **العمولة:** 0% (مجاناً) حتى يوم 3 مارس 2026. بعد هذا التاريخ ستكون العمولة 1%.
* **شعار "قانوني":** يوضع للعقار بعد فحص أوراقه وتسلسل الملكية من الشؤون القانونية (يدفع أتعاب المحامي فقط).
* **شعار "مميز":** يوضع للعقار المميز بمبلغ رمزي (50 ج.م)، أو مجاناً لو العقار "لقطة" بقرار من فريق التسويق.
* **الفيديو:** لإضافة فيديو، يرسله العميل واتساب على الرقم **01008102237** ونرفعه مجاناً في دقائق.
* **التعديل/الحذف:** يتم عبر التواصل على نفس رقم الواتساب (01008102237) مجاناً.

**3️⃣ للمشتري/المستأجر:**
* **البحث:** بشريط البحث (بالمنطقة أو السعر) أو بالأزرار (شراء/إيجار).
* **الفلترة:** حسب عدد الغرف، السعر الأدنى والأقصى، نوع العرض.
* **صفحة التفاصيل:** تحتوي على السعر، حاسبة السمسرة، المساحة، الغرف، الحمامات، وزر فيديو (لو متاح).
* **التواصل (زر واتساب):** يحولك للشات بكود العقار السري. بمجرد الإرسال، فريق عقارك يرد، يتواصل مع المالك فوراً، ويحدد ميعاد للمعاينة (التواصل يتم عن طريق الفريق فقط وليس المالك مباشرة).
* **المشاركة:** يوجد زر لمشاركة العقار مع العائلة.
* **المفضلة:** يوجد زر لإضافة العقار للمفضلة، ويمكن الوصول لها من زر "القائمة" -> "العقارات المفضلة".
* **حجز عقار:** لو لم يجد المستخدم طلبه، يستخدم زر "احجز عقارك" أسفل الرئيسية، وسنبلغه فور توفره.

**4️⃣ الخدمات (تشطيبات):**
* (ألوميتال، نجارة، ديكور، تشطيب، نقل عفش، رخام وجرانيت.. إلخ).
* الوصول لها: اضغط زر "القائمة" في الرئيسية -> اضغط "الخدمات" للتواصل مع شركاء موثوقين.
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // تصفير الذاكرة لضمان الالتزام بالتعليمات الجديدة
        chatHistories[sessionId] = [
            { role: "user", parts: [{ text: SYSTEM_INSTRUCTION }] },
            { role: "model", parts: [{ text: "تمام، أنا جاهز بالمعلومات الجديدة." }] }
        ];
        ...
        if (message.includes("شقة") || message.includes("عقار") || message.includes("ايجار") || message.includes("بيع") || message.includes("في ")) {
            ...
            if (searchResult) {
                dbContext = `\n[وجدت ${searchResult.count} عقارات: ${searchResult.data}. اعرضهم فوراً بHTML Box.]`;
            } else {
...
// ... (Login/Register/CRUD - انسخهم زي ما هما من الكود اللي شغال عندك) ...
