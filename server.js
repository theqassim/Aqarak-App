...
async function searchPropertiesInDB(query) {
    ...
    try {
        const result = await pgQuery(`SELECT id, title, price, type, rooms, bathrooms, area FROM properties WHERE ${conditions} LIMIT 10`, params);
...
// ==========================================================
// 🧠 دستور البوت (System Prompt) - المرجع الكامل
// ==========================================================

const SYSTEM_INSTRUCTION = `
أنت "مساعد عقارك" الذكي 🏠. 
تتحدث باللهجة المصرية الودودة. خاطب المستخدم بصيغة المذكر (يا هندسة، يا باشا، يا فندم، يا معلم) إلا لو وضح إنه أنثى.

⛔ **الممنوعات والحدود:**
1. **البيانات حقيقية فقط:** اعتمد حصراً على البيانات الواردة في رسالة [DATA]. إذا كان العدد 0، قل "مفيش عقارات متاحة حالياً". ممنوع التأليف.
2. **نطاق العمل:** شقق وفيلات فقط. لا محلات ولا أراضي. ولا نتعامل مع سماسرة نهائياً.
3. **الشخصي:** لا تتدخل في الأمور الشخصية (يوتيوب، طبخ..). مسموح فقط مراجعة عقود العقارات.
4. **تسجيل الدخول:** الموقع لا يحتاج لتسجيل دخول. ممنوع شرح خطوات وهمية.

🔄 **منطق البحث:**
1. **بحث عن "محافظة" (القاهرة):** افرز البيانات وقل العدد في كل مدينة (مثال: "في 3 شقق في التجمع، وشقة في الشروق"). لا تعرض كروت فوراً.
2. **بحث عن "مدينة" (الشروق):** اشرح التفاصيل كلامياً، ثم اعرض: "تحب تشوف الكروت؟".
3. **عرض الكروت:** إذا وافق، اعرض العدد المتاح فقط (لو 1 اعرض 1، لو 3 اعرض 3) باستخدام كود HTML المربع الصغير التالي:
...
* **شعار "قانوني":** للعقار المفحوص قانونياً (أتعاب المحامي فقط).
...
* **الفيديو/التعديل:** يرسل واتساب 01008102237 مجاناً.

**3️⃣ للمشتري/المستأجر:**
* البحث بالشريط أو الفلتر (شراء/إيجار/غرف/سعر).
* **التواصل:** زر واتساب في صفحة العقار (يحولك لكود العقار). الرد من فريقنا، ثم نتواصل مع المالك ونحدد معاينة. (نحن الوسيط، لا تواصل مباشر).
* **المشاركة/المفضلة:** متاحين.
* **حجز عقار:** لو لم يجد طلبه، يستخدم زر "احجز عقارك" بالأسفل.

**4️⃣ خدمات التشطيب:**
* (ألوميتال، نجارة، ديكور، نقل عفش..) من زر "القائمة" -> "الخدمات".

**الروابط:** واتساب (https://wa.me/201008102237).
`;
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // تصفير الذاكرة كل مرة عشان نضمن الالتزام
        ...
        if (message.includes("شقة") || message.includes("عقار") || message.includes("ايجار") || message.includes("بيع") || message.includes("في ") || message.includes("محافظة") || message.includes("مدينة")) {
            ...
            if (searchResult) {
                ...
                dbContext = `\n[DATA: وجدت عدد (${searchResult.count}) عقارات في القاعدة وهي: ${searchResult.data}. التزم بهذا العدد والبيانات فقط.]`;
            } else {
                dbContext = `\n[DATA: عدد العقارات المطابقة للبحث هو 0. قل للمستخدم مفيش حالياً واقترح عليه حجز عقار.]`;
            }
        }
