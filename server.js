...
async function setupAI() {
    console.log("⏳ جارٍ تجهيز المساعد الذكي (تم تحسين دقة الفهم)...");

    // أ. فصل التحية عن الاسم عن الوقت
    // 1. عامل ايه (Health)
    ...
    manager.addDocument('ar', 'ايه الاخبار', 'smalltalk.greetings');
    manager.addAnswer('ar', 'smalltalk.greetings', 'بخير والحمد لله! 🦾 جاهز لمساعدتك.');

    // 2. اسمك ايه (Identity) - زدنا الأمثلة هنا عشان يفهم الفرق
    manager.addDocument('ar', 'انت مين', 'agent.who');
    manager.addDocument('ar', 'عرف نفسك', 'agent.who');
    manager.addDocument('ar', 'اسمك ايه', 'agent.who');
    manager.addDocument('ar', 'ما اسمك', 'agent.who');
    manager.addDocument('ar', 'اي اسمك', 'agent.who');
    manager.addDocument('ar', 'مين انت', 'agent.who');
    manager.addAnswer('ar', 'agent.who', 'أنا بوت عقارك الذكي 🏠. أساعدك في البحث عن الشقق والإجابة عن استفساراتك.');

    // 3. الساعة كام (Time vs Work Hours)
    // ده عشان لما حد يسأل الساعة كام ميردش بمواعيد العمل
    manager.addDocument('ar', 'الساعة كام', 'smalltalk.time');
    manager.addDocument('ar', 'كم الساعة', 'smalltalk.time');
    manager.addDocument('ar', 'الوقت كام', 'smalltalk.time');
    manager.addAnswer('ar', 'smalltalk.time', 'أنا بوت لا أرتدي ساعة 😅، لكن الموقع يعمل 24 ساعة لخدمتك!');

    // 4. مواعيد العمل (Work Hours)
    manager.addDocument('ar', 'مواعيد العمل', 'site.hours');
    manager.addDocument('ar', 'بتفتحوا امتى', 'site.hours');
    manager.addDocument('ar', 'شغالين للساعة كام', 'site.hours');
    manager.addAnswer('ar', 'site.hours', 'موقع عقارك متاح 24 ساعة طوال أيام الأسبوع. يمكنك التواصل مع البائعين في أي وقت.');


    // ب. سيناريوهات الموقع الأساسية
    manager.addDocument('ar', 'ازاي اعمل حساب', 'site.auth');
    manager.addDocument('ar', 'تسجيل دخول', 'site.auth');
    manager.addDocument('ar', 'انشاء حساب', 'site.auth');
    manager.addDocument('ar', 'دخول', 'site.auth'); // كلمة دخول لوحدها هنا عشان يفهم انها مش شتيمة
    ...
    await manager.train();
    manager.save();
    console.log("✅ تم تدريب البوت (تم حل مشاكل الشتائم وتشابه الجمل)");
}

setupAI();

// ==========================================================
// --- API الشات (المنطق المعدل) ---
// ==========================================================
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // 1. فلتر الشتائم الذكي (Smart Profanity Filter)
        // بنقسم الجملة لكلمات ونقارن كل كلمة لوحدها عشان نتجنب "دخول" = "خول"
        const messageWords = message.split(/\s+/); // تقسيم الجملة لمسافات
        const containsBadWord = messageWords.some(word => BAD_WORDS.includes(word));
        
        // أو لو الكلمة السيئة موجودة لوحدها بالضبط
        ...
        if (response.intent === 'db.search' || message.includes('شقة') || message.includes('عقار') || message.includes('في') || message.includes('بكام')) {
            ...
            // شيلنا كلمة "الساعة" من هنا عشان ميعملش بحث لما حد يسأل الساعة كام
            ...
            // شرط: لازم الكلمة تكون أكبر من حرفين ومش بتسأل عن الوقت
        ...
        res.json({ reply: "عذراً، لم أفهم سؤالك بدقة. 😅\nيمكنك البحث عن العقارات بكتابة اسم المنطقة، أو التواصل معنا واتساب." });
