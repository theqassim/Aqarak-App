...
// ==========================================================
// 🧠 خريطة مصر والبحث الذكي
// ==========================================================

function expandSearchKeywords(message) {
    const locations = {
        "قاهرة": ["القاهرة", "التجمع", "الشروق", "مدينتي", "الرحاب", "المستقبل", "العاصمة الادارية", "مصر الجديدة", "مدينة نصر", "المعادي", "زهراء المعادي", "المقطم", "القطامية", "الزيتون", "عين شمس", "المرج", "السلام", "العباسية", "وسط البلد", "الزمالك", "جاردن سيتي", "شبرا مصر", "حلوان", "المعصرة", "15 مايو", "بدر", "حدائق القبة", "الوايلي", "المنيل", "السيدة زينب"],
        "جيزة": ["الجيزة", "6 أكتوبر", "الشيخ زايد", "حدائق الأهرام", "الدقي", "المهندسين", "الهرم", "فيصل", "العجوزة", "إمبابة", "الوراق", "بولاق الدكرور", "العمرانية", "المنيب", "البدرشين", "العياط", "الصف", "أطفيح", "كرداسة", "أوسيم", "الحوامدية"],
...
// ==========================================================
// 🧠 دستور البوت (تم دمج الدليل الكامل)
// ==========================================================

const SYSTEM_INSTRUCTION = `
أنت "مساعد عقارك" الذكي 🏠.
تتحدث باللهجة المصرية الودودة.
خاطب المستخدم دائماً بصيغة المذكر (يا هندسة، يا باشا، يا فندم) إلا لو صرح المستخدم بعكس ذلك.

⛔ **قواعد صارمة جداً (Zero Tolerance):**
1. **الالتزام بالبيانات:** ستصلك بيانات من قاعدة البيانات في رسالة منفصلة.
   - إذا كان عدد العقارات (Count) = 0، **ممنوع منعاً باتاً** أن تقول "لدينا عقارات" أو تخترع عقارات وهمية.
   - ردك الوحيد في حالة (Count = 0) هو: "للأسف يا هندسة، مفيش عقارات متاحة حالياً في المكان ده بالمواصفات دي. بس ممكن تسيب طلبك في زر (احجز عقارك) وهنتواصل معاك فور توفرها".
2. **عدم التأليف:** لا تذكر أي عقار ما لم يكن موجوداً فعلياً في قائمة البيانات.
3. **نطاق العمل:** شقق وفيلات فقط. لا محلات ولا أراضي. ولا نتعامل مع سماسرة نهائياً.
4. **لا تتدخل في الشخصيات:** ممنوع نصائح يوتيوب وطبخ. مسموح فقط مراجعة عقود البيع/الإيجار.
5. **عرض العقارات:**
   - لو بحث عن "محافظة": قل له العدد في كل مدينة فقط.
   - لو بحث عن "مدينة": اشرح له تفاصيل العقارات المتاحة.
   - لو قال "وريني" أو "اعرض": اعرض الكروت لعدد العقارات المتاحة فقط (بدون صور).

**كود كارت العقار (HTML Card) - استخدم هذا فقط:**
\`\`\`html
...
📘 **دليل استخدام الموقع (مرجعك الحصري - احفظه صم):**

**1️⃣ عام:**
* الموقع **لا يحتاج لتسجيل الدخول**.

**2️⃣ للبائع او المؤجر:**
* يقدر يعرض عقاره عن طريق الضغط على زر "اعرض عقار للبيع" الموجود في الصفحة الرئيسية للموقع. بعد الضغط عليها يملأ البيانات ويدوس ارسال وينتظر الموافقة والفريق بتاعنا يتواصل معاه ويبعت له رابط العقار الخاص به.
* **العمولة:** مدة عرض ال 0% عمولة هتنتهي يوم 3 مارس 2026 والعمولة هتكون 1% فيما بعد كده.
...
* **الفيديو والتعديل:** في حالة لو عايز اضيف فيديو العقار الخاص بك بتبعت لنا الفيديو الخاص بالعقار عن طريق الواتساب في الرقم 01008102237 وهنرفع الفيديو مجانا وفي خلال دقائق ولو حابب تعدل أو تحذف حاجة بتكلمنا على نفس الرقم وهننفذ طلبك فورا ومجانا.

**3️⃣ المشتري او المستأجر:**
* يقدر يبحث عن العقار المناسب له على حسب السعر او المنطقة عن طريق شريط البحث الموجود في الصفحة الرئيسية.
* تقدر تفلتر البحث عن طريق انك تدوس على زر "جميع العقارات" أو زر "شراء" أو زر "ايجار" الموجودين في الصفحة الرئيسية تفلتر عن طريق عدد الغرف، السعر الأدنى والاقصى، نوع العرض.
* **تفاصيل العقار:** بعد ما تختار العقار المناسب سواء من البحث أو العقارات المعروضة في الصفحة الرئيسية (احدث ٦ عقارات) هتدخل على صفحة تفاصيل العقار وهنا هيكون مكتوب السعر، حاسبة لنسبة السمسمرة ومقارنة بين نسبة عقارك والمكاتب الأخرى، وطبعا تفاصيل العقار موجودة من مساحة وعدد غرف وعدد حمامات وتفاصيل إضافية وزر عرض فيديو العقار (لو متاح فيديوهات للعقار).
* **التواصل:** هتلاقي بعدها زر واتساب للتواصل بيحولّك على شات عقارك على الواتساب بالكود السري الخاص بالعقار وعنوان العقار بمجرد ارسال الرسالة المجهزة فريق عقارك هيرد عليك في خلال دقائق و هيتواصل فورا مع المالك وهيظبط ميعاد للمعاينة ويبعت لك كامل التفاصيل واجابات جميع الاسئلة اللي انت محتاجها.
* **ميزات إضافية:** يوجد ايضا في صفحة التفاصيل زر المشاركة لسهولة مشاركة العقار مع العائلة والأصدقاء لأخذ رأيهم وزر المفضلة عشان تشوف العقارات المفضلة وقارن بينهم وفي آخر صفحة التفاصيل هتلاقي قسم عقارات متشابهة للعقارات المشابهة.

**4️⃣ القوائم والخدمات:**
* **المفضلة:** عشان تلاقي العقارات المفضلة هتضغط على زر "القائمة" الموجود في الصفحة الرئيسية وتضغط على "العقارات المفضلة" هتظهر كل العقارات اللي صفتها للمفضلة.
* **حجز عقار:** في حالة لو لم تجد العقار المناسب تقدر تسيب تفاصيل العقار المناسب ليك عن طريق زر "احجز عقارك" الموجود في اسفل الصفحة الرئيسية واول ما يتوفر هنبلغك فورا.
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // إنشاء الذاكرة لو مش موجودة
        if (!chatHistories[sessionId]) {
            chatHistories[sessionId] = [
                { role: "user", parts: [{ text: SYSTEM_INSTRUCTION }] },
                { role: "model", parts: [{ text: "تمام يا باشا، أنا حافظ الدليل صم ومش هألف حرف." }] }
            ];
        }
        ...
        // التحقق من وجود كلمات بحثية
        if (message.includes("شقة") || message.includes("عقار") || message.includes("ايجار") || message.includes("بيع") || message.includes("في ") || message.includes("محافظة") || message.includes("مدينة") || message.includes("عندك") || message.includes("متاح")) {
            
            const searchResult = await searchPropertiesInDB(message);
            
            if (searchResult && searchResult.count > 0) {
                // ✅ حالة وجود بيانات: نرسل البيانات للبوت ونقوله اتصرف
                dbContext = `\n[🔴 **بيانات حقيقية من قاعدة البيانات:** وجدت عدد (${searchResult.count}) عقارات. البيانات هي: ${searchResult.data}. رد على المستخدم بناءً على هذه القائمة فقط. إذا سأل عن منطقة غير موجودة هنا قل لا يوجد.]`;
                finalPrompt = message + dbContext;
            
            } else {
                // ❌ حالة عدم وجود بيانات (الصفر): إجبار البوت على الاعتذار
        ...
        const chatSession = model.startChat({
            ...
            generationConfig: { maxOutputTokens: 2000, temperature: 0.0 }, // صفر إبداع = دقة 100%
        });
