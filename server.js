...
// ==========================================================
// 🧠 البحث الذكي الشامل (جميع المحافظات)
// ==========================================================
...
// ==========================================================
// 🧠 دستور البوت (System Prompt) - المرجع الكامل والشامل
// ==========================================================
...
4. **عرض العقارات:**
   ...
   ⚠️ **قواعد الإعدام (Zero Tolerance Rules):**
...
2. **عدم التأليف:** لا تذكر أي عقار (لا في التجمع ولا أكتوبر ولا غيره) ما لم يكن موجوداً فعلياً في قائمة البيانات التي وصلتك الآن.
3. **نطاق العمل:** شقق وفيلات فقط. لا محلات ولا أراضي. ولا نتعامل مع سماسرة.
4. **المواضيع:** عقارات فقط (بيع/شراء/إيجار/عقود). ممنوع طبخ أو رياضة.

🔄 **سيناريو العرض:**
1. إذا بحث عن "محافظة" (القاهرة): قل فقط: "متاح حالياً [العدد] عقارات في [اسم المدينة]" بناءً على البيانات.
2. إذا بحث عن "مدينة" (الشروق) ووجد عقارات: اشرح تفاصيلها باختصار، ثم قل: "تحب تشوف الكارت؟".
3. **عرض الكارت:** استخدم الكود التالي فقط (بدون صور، قابل للضغط):
   \`\`\`html
...
* **المفضلة:** عشان تلاقي العقارات المفضلة هتضغط على زر "القائمة" الموجود في الصفحة الرئيسية وتضغط على "العقارات المفضلة" هتظهر كل العقارات اللي ضفتها للمفضلة.
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        chatHistories[sessionId] = [
            ...
            { role: "model", parts: [{ text: "تمام يا باشا، أنا حافظ الدليل صم." }] }
        ];
        ...
        if (message.includes("شقة") || message.includes("عقار") || message.includes("ايجار") || message.includes("بيع") || message.includes("في ") || message.includes("محافظة") || message.includes("مدينة") || message.includes("عندك") || message.includes("متاح")) {
            ...
            if (searchResult && searchResult.count > 0) {
                dbContext = `\n[🔴 **بيانات حقيقية:** وجدت عدد (${searchResult.count}) عقارات. البيانات هي: ${searchResult.data}. رد بناءً على هذه القائمة فقط.]`;
        ...
        const chatSession = model.startChat({
            ...
            generationConfig: { maxOutputTokens: 2000, temperature: 0.0 }, // صفر إبداع = التزام كامل
        });
