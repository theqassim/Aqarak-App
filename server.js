...
// -----------------------------------------------------
// 2. قاعدة البيانات (مع التحديث التلقائي للأعمدة)
// -----------------------------------------------------
...
// 🔥 إنشاء الجداول وتحديث الأعمدة الناقصة تلقائياً 🔥
async function createTables() {
    const queries = [
        // لاحظ إضافة sellerName و sellerPhone في تعريف الجدول
    ...
    try {
        ...
        // 🛑 هذا الكود يضمن إضافة الأعمدة لو كانت قاعدة البيانات قديمة
        ...
        console.log('✅ Tables synced. Columns sellerName/sellerPhone guaranteed.');
    } catch (err) {
...
// منع فتح index.html تلقائياً
app.use(express.static(path.join(__dirname, 'public'), { index: false }));

// -----------------------------------------------------
// 4. مسارات API (أهم جزء: النشر)
// -----------------------------------------------------

// ✅ مسار نشر الطلب (ينقل البيانات من seller_submissions إلى properties)
app.post('/api/admin/publish-submission', async (req, res) => {
    ...
    try {
        // 1. جلب بيانات الطلب من جدول seller_submissions
        ...
        // 2. إدخال البيانات في جدول properties
        // لاحظ: نأخذ sub.sellerName ونضعه في العمود sellerName
        ...
        const params = [
            ...
            sub.propertyType, 
            hiddenCode,
            sub.sellerName,  // ✅ هنا يتم نقل الاسم
            sub.sellerPhone  // ✅ هنا يتم نقل الرقم
        ];
...
app.post('/api/add-property', uploadProperties.array('propertyImages', 10), async (req, res) => {
    ...
    const params = [
        ...
        "الإدارة (يدوي)", // sellerName الافتراضي
        ADMIN_EMAIL      // sellerPhone الافتراضي
    ];
...
// باقي المسارات (GET, DELETE, Auth, Favorites) - كما هي
app.get('/api/admin/seller-submissions', async (req, res) => {
    try { const r = await pgQuery("SELECT * FROM seller_submissions WHERE status = 'pending' ORDER BY \"submissionDate\" DESC"); res.json(r.rows); } catch (err) { throw err; }
});
app.get('/api/admin/property-requests', async (req, res) => {
    try { const r = await pgQuery("SELECT * FROM property_requests ORDER BY \"submissionDate\" DESC"); res.json(r.rows); } catch (err) { throw err; }
});
...
app.delete('/api/admin/property-request/:id', async (req, res) => {
    try { await pgQuery(`DELETE FROM property_requests WHERE id = $1`, [req.params.id]); res.json({ message: 'تم الحذف' }); } catch (err) { throw err; }
});
app.post('/api/submit-seller-property', uploadSeller.array('images', 10), async (req, res) => {
    ...
    try { await pgQuery(sql, params); res.status(200).json({ success: true, message: 'تم الاستلام' }); } catch (err) { throw err; }
});
app.post('/api/request-property', async (req, res) => {
    ...
    try { await pgQuery(`INSERT INTO property_requests (name, phone, email, specifications, "submissionDate") VALUES ($1, $2, $3, $4, $5)`, [name, phone, email, specifications, new Date().toISOString()]); res.status(200).json({ success: true, message: 'تم الاستلام' }); } catch (err) { throw err; }
});
...
app.post('/api/favorites', async (req, res) => {
    ...
    if (!userEmail || !propertyId) return res.status(400).json({ message: 'بيانات ناقصة' });
    try { await pgQuery(`INSERT INTO favorites (user_email, property_id) VALUES ($1, $2)`, [userEmail, propertyId]); res.status(201).json({ success: true }); } 
    catch (err) { if (err.code === '23505') return res.status(409).json({ message: 'موجودة' }); throw err; }
});
app.delete('/api/favorites/:propertyId', async (req, res) => {
    const { userEmail } = req.query;
    if (!userEmail) return res.status(400).json({ message: 'الإيميل مطلوب' });
    try { const result = await pgQuery(`DELETE FROM favorites WHERE user_email = $1 AND property_id = $2`, [userEmail, req.params.propertyId]); res.json({ success: true }); } catch (err) { throw err; }
});
app.get('/api/favorites', async (req, res) => {
    const { userEmail } = req.query;
    if (!userEmail) return res.status(400).json({ message: 'الإيميل مطلوب' });
    const sql = `SELECT p.id, p.title, p.price, p.rooms, p.bathrooms, p.area, p."imageUrl", p.type, f.id AS favorite_id FROM properties p JOIN favorites f ON p.id = f.property_id WHERE f.user_email = $1 ORDER BY f.id DESC`;
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        if(await bcrypt.compare(password, r.rows[0].password)) res.json({success:true, role:r.rows[0].role}); else res.status(401).json({message:'بيانات خاطئة'});
    } catch (e) { throw e; }
});
app.post('/api/register', async (req, res) => {
    ...
    } catch (error) { if (error.message.includes('unique constraint')) return res.status(400).json({ message: 'مسجل مسبقاً' }); throw error; }
});
