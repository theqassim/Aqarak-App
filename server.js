...
// ==========================================================
// 🧠 العقل المدبر (Gemini AI Logic) - التحديث الكامل للتعليمات
// ==========================================================
...
🚨 **قواعد الرد الصارمة (System Rules):**
1. **الاختصار:** جاوب في جملة أو جملتين بالكتير (خير الكلام ما قل ودل).
2. **سياسة التواصل (هام جداً):** إذا سأل العميل عن "التواصل مع المالك"، وضح له فوراً وبشكل حاسم أن **"التواصل بيتم حصراً عن طريق فريق عقارك، وإحنا اللي بننسق الميعاد مع المالك"**. لا يوجد تواصل مباشر بين العميل والمالك.
3. **الروابط:** استخدم هذه الروابط *فقط* عند الحاجة للتواصل:
   * **واتساب:** https://wa.me/201008102237
   * **فيسبوك:** https://www.facebook.com/share/17b14ZTvd9/
   * **انستجرام:** https://instagram.com/aqarak.eg
4. **التنسيق:** استخدم النجوم (**) لتمييز الكلمات الهامة (مثل السعر أو الاسم) لتظهر بخط عريض.
...
**2️⃣ للبائع/المؤجر:**
* لعرض عقار: زر "اعرض عقار للبيع" بالرئيسية -> املأ البيانات -> دوس إرسال -> انتظر موافقة الفريق.
* **العمولة:** 0% (مجاناً) حتى 3 مارس 2026، بعدها ستكون 1%.
* **شعار "قانوني":** بيتحط للعقار بعد فحص أوراقه من الشؤون القانونية (يتم دفع أتعاب المحامي فقط).
* **شعار "مميز":** بمبلغ رمزي (50 ج.م)، أو مجاناً لو العقار "لقطة".
* **الفيديو:** ابعته واتساب على 01008102237 وهنرفعه مجاناً في دقايق.

**3️⃣ للمشتري/المستأجر:**
* **البحث:** بشريط البحث (بالمنطقة أو السعر).
* **الفلترة:** بأزرار "جميع العقارات"، "شراء"، "إيجار".
* **صفحة التفاصيل:** فيها السعر، المساحة، زر فيديو، و **زر واتساب** (بيحولك لينا بكود العقار عشان نحدد معاينة).
* **العقارات المفضلة:** اضغط "القائمة" -> "العقارات المفضلة".
* **حجز عقار:** لو مش لاقي طلبك، استخدم زر "احجز عقارك" تحت في الرئيسية.

**4️⃣ الخدمات:**
* (ألوميتال، نجارة، تشطيب، نقل عفش..): اضغط "القائمة" -> "الخدمات".
`;

const chatHistories = {};

// دالة البحث في قاعدة البيانات لتعزيز ردود البوت
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // إنشاء معرف جلسة بسيط يعتمد على الكوكيز أو عنوان IP
        ...
        // تهيئة الذاكرة إذا كانت جلسة جديدة
        if (!chatHistories[sessionId]) {
            chatHistories[sessionId] = [
                { role: "user", parts: [{ text: SYSTEM_INSTRUCTION }] },
                { role: "model", parts: [{ text: "تمام يا هندسة، أنا جاهز." }] }
            ];
        }

        // البحث في الداتا بيز إذا كان السؤال عن عقارات
        let dbContext = "";
        if (message.includes("شقة") || message.includes("عقار") || message.includes("ايجار") || message.includes("بيع") || message.includes("في ")) {
            ...
            if (searchResult) {
                dbContext = `\n[استخدم هذه البيانات للرد: ${searchResult}]`;
            } else {
                dbContext = `\n[لم أجد عقارات حالياً تطابق البحث، اقترح عليه زر "احجز عقارك".]`;
            }
        }
        ...
        const chatSession = model.startChat({
            ...
            generationConfig: { maxOutputTokens: 300 }, // تقليل التوكنز لضمان الاختصار
        });
        ...
        // الحفاظ على حجم الذاكرة (آخر 10 رسائل فقط)
        ...
        res.status(500).json({ reply: "معلش يا هندسة، النت تقيل شوية. ممكن تعيد السؤال؟ 😅" });
    }
});

// ==========================================================
// --- باقي الـ API Routes (Login, Register, CRUD) ---
// ==========================================================
