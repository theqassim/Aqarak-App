...
// ✅ دالة حذف الصور من Cloudinary (كانت ناقصة وهي سبب المشكلة)
async function deleteCloudinaryImages(imageUrls) {
    ...
    const publicIds = imageUrls.map(url => {
        try {
            ...
            // تأكد إنك بتستخدم اسم الفولدر الصح
            return `aqarak_submissions/${id}`; 
        } catch (e) { return null; }
    }).filter(id => id !== null);
    ...
    try {
        ...
        if (message.includes("شقة") || message.includes("عقار") || message.includes("ايجار") || message.includes("بيع") || message.includes("في ") || message.includes("محافظة") || message.includes("مدينة") || message.includes("عندك") || message.includes("متاح")) {
            ...
            if (searchResult && searchResult.count > 0) {
                dbContext = `\n[🔴 بيانات حقيقية: وجدت (${searchResult.count}) عقارات: ${searchResult.data}. رد بناءً عليها فقط.]`;
                finalPrompt = message + dbContext;
            } else {
                finalPrompt = `المستخدم بيسأل: "${message}". 
                🔴 تقرير: لا توجد عقارات مطابقة (العدد 0).
                ⚠️ أمر: اعتذر للمستخدم وقل "مفيش عقارات حالياً" واقترح "احجز عقارك".`;
            }
        }
...
app.delete('/api/admin/seller-submission/:id', async (req, res) => { 
    try { 
        ...
        if (r.rows[0]) await deleteCloudinaryImages((r.rows[0].imagePaths || '').split(' | ')); 
