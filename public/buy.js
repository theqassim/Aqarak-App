document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('filter-apply-btn').addEventListener('click', fetchBuyProperties);
});
...
async function fetchBuyProperties() {
    ...
    let apiUrl = '/api/properties?type=buy&';
    ...
    try {
        ...
        if (properties.length === 0) {
            title.innerText = 'لا توجد عقارات للبيع تطابق البحث';
            container.innerHTML = '<p class="empty-message">لم نجد أي عقارات للبيع تطابق بحثك. حاول مرة أخرى أو قم بتغيير الفلاتر.</p>';
            return;
        }

        title.innerText = `تم العثور على ${properties.length} عقار للبيع`;
